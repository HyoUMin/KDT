...
# 【사용 데이터 1 】 --------------------------------------------------------------------- 
# 다음 조건에 맞게 midterm Series와 final Series를 생성하세요. midterm : 중간고사 점수를 저장합니다.
# 인덱스 값 
# 김민수 80 
# 이서연 90 
# 박지훈 75 
...
# 【사용 데이터 2 】 --------------------------------------------------------------------- 
# 다음 조건에 맞게 class_a DataFrame과 class_b DataFrame을 생성하세요. class_a : 아래 학생별 과목 점수를 저장합니다. 
# 이름 국어 영어 수학 
# 김민수 80 85 90 
# 이서연 90 88 95 
# 박지훈 75 79 70 
...
class_a = pd.DataFrame(data1, index = ['김민수', '이서연', '박지훈'])
class_b = pd.DataFrame(data2, index = ['최유진', '정하늘', '한지민'])
# -------------------------------------------------------------------------------------- 
...
# 조건 
# - s1과 s2는 모두 문자열 인덱스를 갖는 Series로 생성하세요. - s1 Series의 인덱스는 A, B, C 이고 데이터는 10, 20, 30 - s2 Seires의 인덱스는 B, C, D 이고 데이터는 1, 2, 3 
...
# → midterm과 final을 이용하여 exam DataFrame을 생성하세요. → exam DataFrame의 컬럼명은 중간, 기말로 지정하세요. → 총점 열을 추가하세요. 
# → 평균 열을 추가하세요. 
